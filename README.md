...
**Phase 1 — feature extraction** (`01_extraccion.ipynb`)
...
```bash
# 1. Install dependencies (and ffmpeg, see section 1)
pip install -r requirements.txt

# 2. Edit AUDIO_DIR and METADATA_DIR in 01_extraccion.ipynb, then run it
#    to produce fma_small_features.csv
jupyter notebook 01_extraccion.ipynb

# 3. Run the modeling notebook end to end to reproduce the tables and figures
jupyter notebook 02_modelado.ipynb
```

To run a notebook non-interactively from the command line instead:

```bash
jupyter nbconvert --to notebook --execute 01_extraccion.ipynb
